...
## Goal
講師が `/admin/` にログインし、トップページの見出し・本文ブロック名・本文を保存だけで更新できるようにする。

## Context
- 編集対象は `content/home-text.json` のセクション見出し、本文ブロック名、本文です。
...
- セクション見出しは18文字以内、本文ブロック名は28文字以内にします。公開ページの見た目を崩さないためです。
- 講師が保存すると Git に `content/home-text.json` がコミットされ、Netlify の build command が公開HTMLを生成します。
- スマホで `/admin/` を開いた場合は、スマホ用の軽量編集画面 `/admin/mobile.html` に自動で切り替えます。
- 編集画面にはログイン中のメールアドレスを表示します。別の人が使う端末では必ずログアウトします。

## Done when
- 講師は `https://abepianoroom.netlify.app/admin/` にログインできる。
- 「トップページ本文」を編集して保存すると、数分後に公開ページへ反映される。
- スマホでは `https://abepianoroom.netlify.app/admin/mobile.html` で本文を読み書きできる。
...
6. 「ページに表示する見出し」「ブロック名」「本文」を直したら「保存」します。
7. 見出しは18文字以内、ブロック名は28文字以内にしてください。
8. 公開ページへの反映には数分かかります。
9. 画面に「ログイン中: メールアドレス」が表示されます。共用端末では、終わったらログアウトしてください。
...
## 編集できる範囲
- セクション見出し: 公開ページの各本文セクションに出る大きな見出し。
- ブロック名: CMS画面で本文ブロックを見分けるための名前。公開ページの見た目には出ません。
- 本文: 公開ページに表示される文章。改行は公開ページの既存ルールどおりに反映されます。
- 変更できないもの: 料金表、曜日表、価格セル、画像、ヒーロー文言。

## スマホ編集画面
- スマホから `/admin/` を開くと `/admin/mobile.html` に自動で移動します。
- スマホ編集画面は本文編集だけに絞り、プレビューやDecapの複雑なサイドバーは表示しません。
- 保存は既存の Netlify Identity / Git Gateway を使います。GitHub連携や公開設定は変更しません。
- 画面上部にログイン中のメールアドレスを表示します。意図しないアドレスが表示された場合はログアウトして入り直します。
