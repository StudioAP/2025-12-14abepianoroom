# content_master_v1

## 役割
- 本文・写真仕様は本ファイルを正本とし、`index.html` 直編集のみで確定しない。
- 編集順序: `content_master_v1.md` 更新 → `index.html` 反映 → `node scripts/check_content_master_consistency.mjs` 合格。
- 差分チェックが `PASS` になるまで、公開反映しない。

## ヒーロー仕様
- title: 安部ピアノルーム
- sub: 大人・子ども向け 自宅個人レッスン
- location: 京都市北区上賀茂（上賀茂・西賀茂・市原エリア）
- mobileSplitTopFirstImage: image/hero-lead-opt.jpg
- mobileSplitBottomImage: image/hero-top-opt.jpg
- rotationImages:
  - image/hero-lead-opt.jpg
  - image/hero-top-opt.jpg
  - image/hero-bottom-opt.jpg

## セクション本文・写真
### 1. ごあいさつ
- textFlow: structured

#### Text 1
```text
初めてだけど弾いてみたい…
昔習っていてまた弾きたくなった…
シニア世代の趣味に…
お腹の赤ちゃんと音楽を楽しみたい…
育休中、乳児を連れて一息つきたい…
教職課程で必要になった…
京都市北区上賀茂（上賀茂・西賀茂・市原エリア）で、ピアノレッスンを行っています。
体験レッスンは1,000円です。
「お問合せ」からお気軽にご質問ください。
```

### 2. 対象・場所
- textFlow: structured

#### Text 1
```text
【対象】
・初心者〜中級くらいの方
・大人は女性の方のみを対象としています

【場所】
京都市北区上賀茂（自宅教室）
バス停「柊野別れ」「ゴルフ場前」柊野小学校 徒歩10分
北大路からバス12〜13分
西賀茂・市原からも通いやすい立地です。

※自宅教室につき住所詳細は、お問合せくださった方にお伝えしております。駐車可。
```

### 3. 料金・曜日・時間
- textFlow: structured

#### Text 1
```text
【★子どもレッスンの、「自宅練習サポート動画」について】

保護者の皆さま・お子さま双方の生活スタイルが多様化する中で、練習時間を確保することが簡単ではないこともあると思います。

自宅で練習が思うように進まない時期があっても、レッスンの時間内で学べることはたくさんあります。

ピアノの上達には継続した練習が欠かせませんが、「練習しなければ」という思いが強くなりすぎると、音楽の楽しさが薄れてしまうことがあります。指導する側としても、皆さんの音楽への興味の芽を大切に育てられるよう、日々指導のあり方を常に振り返るようにしています。

そのような思いから、ご自宅での練習が軌道に乗るサポートとなる、動画を配信いたします。
```

#### Text 2
```text
※補足（高校生まで）：現在、満室につき新規受付を行っておりません。再開の場合は改めてご案内します。
```

### 4. 大人向けへの想い
- textFlow: natural

#### Text 1
```text
ピアノレッスンを始めて約18年。大人向けのレッスンコースを始めました。

私は、娘が0歳だったころ、娘を連れてとあるお稽古ごとに通い、心身ともにとてもリフレッシュできた経験があります。そんな実体験から、（いつか自分もピアノを通して、大人の方がホッとできるような、大人向けのレッスンをご提供できたら...）と、願うようになりました。

ピアノはこつこつと続けて上達していくものですが、仕事や子育てをしていると、続けるのが大変になることもあります。

都合がつかない時期には休み、時間ができたらまた再開する...というのも「続け方」だと思います。生活に合わせて、歩いたり、立ち止まったり、時には走ってピアノに打ち込んでみたり...
長い長い目で見て、「弾きたい」気持ちを大切に、ピアノを楽しんでいただけたらと思っています。
```

### 5. 教室概要・講師略歴
- textFlow: structured

#### Text 1
```text
〈安部ピアノルーム〉
・京都市北区上賀茂の自宅で2009年開室

〈講師略歴〉
1977年京都市生まれ。大阪音楽大学短期学部卒。
ヤマハ指導・演奏 グレード5級。
卒業後、ピアノレッスンの傍らホテルレストラン奏者、ブライダル奏者、またジャズ奏者としても活動経験あり。
音楽以外に好きなものは料理・お菓子作り、おいしいものを食べること。
```

### 6. 小さいお子様向け（リトミック）
- textFlow: natural

#### Text 1
```text
小さいお子様向けに、リトミッククラスを開講しています。
こちらをご覧ください：
https://kogumarr.netlify.app/

※上記に関しまして、ご不明な点等についてはお気軽にご質問・ご相談ください。
```


<!-- CONTENT_MASTER_DATA_START -->
```json
{
  "schemaVersion": 1,
  "hero": {
    "title": "安部ピアノルーム",
    "sub": "大人・子ども向け 自宅個人レッスン",
    "location": "京都市北区上賀茂（上賀茂・西賀茂・市原エリア）",
    "rotationImages": [
      "image/hero-lead-opt.jpg",
      "image/hero-top-opt.jpg",
      "image/hero-bottom-opt.jpg"
    ],
    "mobileSplitTopFirstImage": "image/hero-lead-opt.jpg",
    "mobileSplitBottomImage": "image/hero-top-opt.jpg"
  },
  "sections": [
    {
      "title": "ごあいさつ",
      "subtitle": "",
      "textFlow": "structured",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "初めてだけど弾いてみたい…",
            "昔習っていてまた弾きたくなった…",
            "シニア世代の趣味に…",
            "お腹の赤ちゃんと音楽を楽しみたい…",
            "育休中、乳児を連れて一息つきたい…",
            "教職課程で必要になった…",
            "京都市北区上賀茂（上賀茂・西賀茂・市原エリア）で、ピアノレッスンを行っています。",
            "体験レッスンは1,000円です。",
            "「お問合せ」からお気軽にご質問ください。"
          ]
        }
      ]
    },
    {
      "title": "対象・場所",
      "subtitle": "",
      "textFlow": "structured",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "【対象】",
            "・初心者〜中級くらいの方",
            "・大人は女性の方のみを対象としています",
            "",
            "【場所】",
            "京都市北区上賀茂（自宅教室）",
            "バス停「柊野別れ」「ゴルフ場前」柊野小学校 徒歩10分",
            "北大路からバス12〜13分",
            "西賀茂・市原からも通いやすい立地です。",
            "",
            "※自宅教室につき住所詳細は、お問合せくださった方にお伝えしております。駐車可。"
          ]
        }
      ]
    },
    {
      "title": "料金・曜日・時間",
      "subtitle": "",
      "textFlow": "structured",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "【★子どもレッスンの、「自宅練習サポート動画」について】",
            "",
            "保護者の皆さま・お子さま双方の生活スタイルが多様化する中で、練習時間を確保することが簡単ではないこともあると思います。",
            "",
            "自宅で練習が思うように進まない時期があっても、レッスンの時間内で学べることはたくさんあります。",
            "",
            "ピアノの上達には継続した練習が欠かせませんが、「練習しなければ」という思いが強くなりすぎると、音楽の楽しさが薄れてしまうことがあります。指導する側としても、皆さんの音楽への興味の芽を大切に育てられるよう、日々指導のあり方を常に振り返るようにしています。",
            "",
            "そのような思いから、ご自宅での練習が軌道に乗るサポートとなる、動画を配信いたします。"
          ]
        },
        {
          "type": "text",
          "lines": [
            "※補足（高校生まで）：現在、満室につき新規受付を行っておりません。再開の場合は改めてご案内します。"
          ]
        }
      ]
    },
    {
      "title": "大人向けへの想い",
      "subtitle": "",
      "textFlow": "natural",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "ピアノレッスンを始めて約18年。大人向けのレッスンコースを始めました。",
            "",
            "私は、娘が0歳だったころ、娘を連れてとあるお稽古ごとに通い、心身ともにとてもリフレッシュできた経験があります。そんな実体験から、（いつか自分もピアノを通して、大人の方がホッとできるような、大人向けのレッスンをご提供できたら...）と、願うようになりました。",
            "",
            "ピアノはこつこつと続けて上達していくものですが、仕事や子育てをしていると、続けるのが大変になることもあります。",
            "",
            "都合がつかない時期には休み、時間ができたらまた再開する...というのも「続け方」だと思います。生活に合わせて、歩いたり、立ち止まったり、時には走ってピアノに打ち込んでみたり...",
            "長い長い目で見て、「弾きたい」気持ちを大切に、ピアノを楽しんでいただけたらと思っています。"
          ]
        }
      ]
    },
    {
      "title": "教室概要・講師略歴",
      "subtitle": "",
      "textFlow": "structured",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "〈安部ピアノルーム〉",
            "・京都市北区上賀茂の自宅で2009年開室",
            "",
            "〈講師略歴〉",
            "1977年京都市生まれ。大阪音楽大学短期学部卒。",
            "ヤマハ指導・演奏 グレード5級。",
            "卒業後、ピアノレッスンの傍らホテルレストラン奏者、ブライダル奏者、またジャズ奏者としても活動経験あり。",
            "音楽以外に好きなものは料理・お菓子作り、おいしいものを食べること。"
          ]
        }
      ]
    },
    {
      "title": "小さいお子様向け（リトミック）",
      "subtitle": "",
      "textFlow": "natural",
      "blocks": [
        {
          "type": "text",
          "lines": [
            "小さいお子様向けに、リトミッククラスを開講しています。",
            "こちらをご覧ください：",
            "https://kogumarr.netlify.app/",
            "",
            "※上記に関しまして、ご不明な点等についてはお気軽にご質問・ご相談ください。"
          ]
        }
      ]
    }
  ]
}
```
<!-- CONTENT_MASTER_DATA_END -->
